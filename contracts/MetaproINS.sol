...
contract MetaproINSv1 is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    function getWalletIns(
        address _walletAddress
    ) public view returns (WalletIns[] memory) {
    ...
    function getInsDeposits(
        uint256 _insId
    ) public view returns (INSDeposit[] memory) {
        return insDeposits[_insId];
    }

    function getInsWalletDeposits(
        uint256 _insId,
        address _walletAddress
    ) public view returns (INSDeposit[] memory) {
    ...
    function getBatchIns(
        uint256[] memory _insIds
    ) public view returns (INSConfiguration[] memory) {
    ...
    function getInsTokenContractAddress(
        uint256 _insId
    ...
    function finishAndgetDeposistAmount(
        uint256 _insId,
        address participant
    ) private returns (uint256) {
